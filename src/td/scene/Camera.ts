module td.scene {
    export class Camera {
        /*查看范围扩充*/
        static LOGIC_INNER_LOOK: number = 200;
        ...
        private __location(newX: number, newY: number): void {
            ...
            //左线
            // this.bufferLeft = this.width > this.map_width_px ? this._x - this.centerPointX : this.centerPointX - this._x;
            this.bufferLeft = this._x - this.centerPointX;
            //右线
            this.bufferRight = this.bufferLeft + this.bufferWidth;
            //上线
            // this.bufferTop = this.height > this.map_height_px ? this._y - this.centerPointY : this.centerPointY - this._y;
        ...
        getScenePxByCellY(y: number): number {
            return y * SceneRes.CELL_HEIGHT - this.bufferTop + this.bufferOffsetY;
        }
        ...
        getCellXByScene(x: number): number {
            var v: number = x + this.bufferLeft - this.bufferOffsetX;
		 ...
		 * @return 
		 */
        getCellYByScene(y: number): number {
            var v: number = y + this.bufferTop - this.bufferOffsetY;
        ...
        private updateModeFollow(): void {
            ...
            //设置窗口位置 通过主玩家的实际坐标位置，得到屏幕中央偏移及格子中央偏移		
            this.__location(this._worldPostion.x * SceneRes.CELL_WIDTH, this._worldPostion.y * SceneRes.CELL_HEIGHT);
        }
